...
const features = [
  {
    ...
    title: 'AI Operator Agent',
    description:
      'A fully autonomous marketing agent that lives inside your CRM. Describe what you need in plain English — it queries data, builds segments, and drafts campaigns using real tool calls.',
    gradient: 'from-violet-500 to-purple-500',
    lightBg: 'bg-violet-50',
    lightBorder: 'border-violet-100',
    lightText: 'text-violet-600',
  ...
  },
  {
    ...
    title: 'Human-in-the-Loop Safety',
    description:
      'Read-only operations execute automatically. Write operations (creating segments, drafting campaigns) always pause and ask for your explicit approval before executing.',
    gradient: 'from-emerald-500 to-green-500',
    lightBg: 'bg-emerald-50',
    lightBorder: 'border-emerald-100',
    lightText: 'text-emerald-600',
  ...
  },
  {
    ...
    title: 'Multi-Channel Delivery',
    description:
      'Send campaigns via WhatsApp, Email, SMS, or RCS. A dedicated Channel Service microservice simulates realistic delivery with webhook callbacks for real-time status updates.',
    gradient: 'from-blue-500 to-cyan-500',
    lightBg: 'bg-blue-50',
    lightBorder: 'border-blue-100',
    lightText: 'text-blue-600',
  ...
  },
  {
    ...
    title: 'Funnel Analytics',
    description:
      'Track the complete delivery funnel: Sent → Delivered → Opened → Clicked → Converted. AI auto-generates strategic insights and recommendations from your campaign data.',
    gradient: 'from-orange-500 to-amber-500',
    lightBg: 'bg-orange-50',
    lightBorder: 'border-orange-100',
    lightText: 'text-orange-600',
  ...
  },
  {
    ...
    title: 'AI Content Generation',
    description:
      'Generate campaign titles, subject lines, and personalized messages with a single click. The AI adapts tone and content based on segment demographics and channel best practices.',
    gradient: 'from-pink-500 to-rose-500',
    lightBg: 'bg-pink-50',
    lightBorder: 'border-pink-100',
    lightText: 'text-pink-600',
  ...
  },
  {
    ...
    title: 'Real-time Notifications',
    description:
      'Every significant action triggers an in-app notification with unread badges. Campaign dispatch results with exact sent/failed/opened counts are pushed live to your notification center.',
    gradient: 'from-indigo-500 to-blue-600',
    lightBg: 'bg-indigo-50',
    lightBorder: 'border-indigo-100',
    lightText: 'text-indigo-600',
...
const cardVariants: Variants = {
  ...
  visible: {
    ...
    transition: {
      duration: 0.5,
      ease: 'easeOut', // safest option across all Framer Motion versions
    },
  },
}
    ...
    <section id="features" className="py-24 lg:py-32">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          ...
          <p className="mt-5 text-lg text-[#64748B] leading-relaxed">
            Six pillars of intelligence — from autonomous agents to real-time
            delivery tracking — working together to automate your entire
            marketing pipeline.
          </p>
        </motion.div>
        ...
        >
          {features.map((feature) => (
            ...
            >
              <div
                className={`inline-flex items-center justify-center w-12 h-12 rounded-xl ${feature.lightBg} ${feature.lightBorder} border mb-6`}
              >
                <feature.icon
                  className={`w-6 h-6 ${feature.lightText}`}
                />
              </div>

              <span
                className={`inline-block px-2.5 py-0.5 rounded-md text-[10px] font-semibold uppercase tracking-wider ${feature.lightBg} ${feature.lightText} mb-3`}
              >
                {feature.tag}
              </span>
              ...
              <div className="flex flex-wrap gap-1.5">
                {feature.highlights.map((h) => (
                  <span
                    key={h}
                    className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 border border-slate-200/60"
                  >
                    {h}
                  </span>
                ))}
              ...
              <div
                className={`mt-5 inline-flex items-center gap-1.5 text-sm font-semibold ${feature.lightText} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
              >
                Learn more
                <ArrowUpRight className="w-4 h-4" />
              </div>
              ...
              <div
                className={`absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r ${feature.gradient} rounded-t-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
              />
            </motion.div>
          ))}
        </motion.div>
